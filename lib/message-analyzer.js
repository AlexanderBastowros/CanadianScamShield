...
import {
  hasGibberishLabel, isRandomLocalPart, labelCount, hasAbusedTld, hasDigitMixedLabel,
...
export function parseSenderLine(fromString) {
  ...
  const local = email ? email.split('@')[0] ?? null : null;
  return { displayName, email, domain, local };
}

// ---------------------------------------------------------------------------
// Sender-domain reputation (throwaway / machine-generated address detection)
// ---------------------------------------------------------------------------

const REP_EXPLAIN = {
  random_sender_domain: {
    en: 'The sender’s domain looks randomly generated, not like a real company.',
    fr: 'Le domaine de l’expéditeur semble généré au hasard, pas celui d’une vraie entreprise.',
  },
  random_sender_local_part: {
    en: 'The part of the address before the @ looks randomly generated.',
    fr: 'La partie de l’adresse avant le @ semble générée au hasard.',
  },
  deep_subdomain_chain_sender: {
    en: 'The sender’s domain is unusually long and complex — a sign of a throwaway domain.',
    fr: 'Le domaine de l’expéditeur est anormalement long et complexe — un signe de domaine jetable.',
  },
  abused_sender_tld: {
    en: 'The sender’s domain uses an uncommon ending often used by scammers.',
    fr: 'Le domaine de l’expéditeur utilise une terminaison rare souvent employée par les fraudeurs.',
  },
  digit_mixed_sender_domain: {
    en: 'The sender’s domain mixes random letters and numbers.',
    fr: 'Le domaine de l’expéditeur mélange des lettres et des chiffres au hasard.',
  },
};

/**
 * Scores the sender's address/domain for throwaway-domain signals. Only meant
 * for UNVERIFIED senders (a domain matching the whitelist/known senders is not
 * scored). Individually each signal is modest so a single quirk never raises a
 * banner on its own; a real throwaway domain trips several and adds up fast.
 *
 * @returns {Array<{id:string, weight:number, explanation:string}>}
 */
function scoreSenderReputation(sender, lang) {
  const out = [];
  const host = sender.domain;
  if (!host) return out;
  const push = (id, weight) =>
    out.push({ id, weight, explanation: REP_EXPLAIN[id][lang] || REP_EXPLAIN[id].en });

  if (hasGibberishLabel(host)) push('random_sender_domain', 30);
  if (isRandomLocalPart(sender.local)) push('random_sender_local_part', 25);
  // >= 5: legit marketing mail often uses 4-label hosts (e.mail.brand.com);
  // real throwaway chains (a.b.c.example.biz.id) run deeper.
  if (labelCount(host) >= 5) push('deep_subdomain_chain_sender', 20);
  if (hasAbusedTld(host)) push('abused_sender_tld', 25);
  if (hasDigitMixedLabel(host)) push('digit_mixed_sender_domain', 10);
...
const PREDICATES = {
  ...
  body_dollar_amount_in_range: (ctx, [min, max]) => {
    // Match the full digit run whether or not it uses comma grouping —
    // "$5000" must parse as 5000, not truncate to 500.
    const amounts = [...ctx.body.matchAll(/\$\s?(\d[\d,]*(?:\.\d{2})?)/g)]
  ...
  link_text_domain_differs_from_href_domain: (ctx) =>
    ctx.anchors.some((a) => {
      ...
      // Strip www. from BOTH sides — <a href="https://www.rbc.com">rbc.com</a>
      // (or www.rbc.com) is the same destination, not a mismatch.
      return !!hrefHost && !!textHost &&
        hrefHost.replace(/^www\./, '') !== textHost.replace(/^www\./, '');
    }),
  ...
  claimed_brand_in_whitelist: (ctx) => {
    ...
    return [...ctx.whitelistDomains].some((d) => termMatches(hay, d));
  },
...
export function analyzeMessage(rawText, options = {}) {
  ...
  // Sender-domain reputation — only for unverified senders. Catches throwaway
  // domains with valid SPF/DKIM ("Trusted Sender") but nonsense addresses, which
  // no content keyword would flag.
  if (!senderVerified) {
    for (const r of scoreSenderReputation(ctx.sender, lang)) {
      firedRules.push(r);
      score += r.weight;
    }
  }

...
export async function checkDomainAgeDays(host) {
  ...
  // A null (failed lookup) gets a short TTL so one transient RDAP outage
  // doesn't suppress the young-domain check for a whole week.
  await cacheSet(cacheKey, ageDays, ageDays === null ? 60 * 60 * 1000 : 7 * DAY_MS);
  return ageDays;
}
...
export async function checkPhishTank(url, appKey) {
  ...
  // Same short-TTL-on-failure policy as the RDAP cache.
  await cacheSet(cacheKey, result, result === null ? 60 * 60 * 1000 : DAY_MS);
  return result;
}

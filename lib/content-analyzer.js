...
export function analyzeContent(
  ...
  for (const [catKey, catDef] of Object.entries(categories)) {
    ...
    // Build the unified term list. We ALWAYS match both English and French
    // terms, regardless of the user's UI language — a scam page can be in either
    // official language and must be caught either way. (`lang` only affects the
    // language of the explanation strings, not what we detect.)
    const termSet = new Set([
      ...(Array.isArray(catDef.terms)    ? catDef.terms    : []),
      ...(Array.isArray(catDef.terms_fr) ? catDef.terms_fr : []),
    ]);

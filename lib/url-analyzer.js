...
function hyphenCountInLabels(hostname) {
  ...
  // As a pragmatic proxy: count hyphens in everything except the last two labels.
  const labelPart = parts.slice(0, -2).join('-');
  return (labelPart.match(/-/g) || []).length;
}
...
export function analyzeUrl(url, {
  ...
  for (const wlDomain of whitelistMap.keys()) {
    ...
    // not at the very end (suffix match = legitimate subdomain, already caught above).
